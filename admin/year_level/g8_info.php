...
//find the section
$grd8_section = mysqli_query($con,"SELECT * FROM grd8 WHERE `grd8_id` = '".$_SESSION['grd8_id']."'");
$g8_result = mysqli_fetch_array($grd8_section);
//section
$section = $g8_result['section'];

// get all advisers in secction
$find_adviser = mysqli_query($con, "SELECT * FROM g8_advisers WHERE `section` = '$section' ORDER BY date_added DESC");

if (isset($_GET['delete'])) {
	# code...
$g8_adviser_id= $_GET["g8_adviser_id"];

		$sql= mysqli_query($con,"DELETE  FROM g8_advisers WHERE g8_adviser_id = '$g8_adviser_id '");
...
if (isset($_GET['edit'])) {
$_SESSION['g8_adviser_id'] = $_GET["g8_adviser_id"];

header("location:g8_edit_info.php");
	}
?>
<!-- start HTML here-->
<!DOCTYPE html>
		...
		<?php echo $output;?>
			...
			<form method="POST" action="#">
				<div class="card-panel adviser-content z-depth-3">
					<div class="row">					
							<div class="col s4">
								<a href="grade8.php" class="btn grey lighten-2 black-text">GO BACK</a>
							</div>
						</div>
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<a href="g8_info.php?delete=1&g8_adviser_id=<?php echo $row["g8_adviser_id"];?>" onclick="return confirm('Do you want to delete this?')" class="tooltipped" data-position="top" data-delay="50" data-tooltip="Delete">
							<i class="fa fa-trash fa-2x icon-pad-5" style="color:#f44336; margin-left:10px;"></i>
						</a>
						<a href="g8_info.php?edit=1&g8_adviser_id=<?php echo $row["g8_adviser_id"];?>" class="tooltipped" data-position="top" data-delay="50" data-tooltip="Edit">
