...
if (isset($_GET['edit'])) {
//session
$_SESSION['grd7_id'] = $_GET["grd7_id"];
//redirect
header("location:edit_section_grd7.php");
	} 

if (isset($_GET['delete'])) {
	# code...
$grd7_id = $_GET["grd7_id"];
// find section
$select_section = mysqli_query($con, "SELECT * FROM grd7 WHERE grd7_id =  $grd7_id");
// fetch result
$result = mysqli_fetch_array($select_section);
//delete section from table grd7
$sql= mysqli_query($con,"DELETE  FROM grd7 WHERE grd7_id = $grd7_id ");
...
header("location:grd7_add_new_adviser.php");
}
	...
	<body>
		...
		<li class="no-padding">
			...
			<?php echo $output; ?>  <!-- Alert Message will display here-->
					<div class="card-panel yellow lighten-1 center" 
						  style="width:170px; 
						         height:100px;">
						<h5 class="yellow-text text-darken-4">
							Grade 7
						</h5>
					</div>
				</div>
			...
			<br>
				<div class="row">
					<div class=" col s1 push-s10 custom-button-top">
						<a href="add_section_grd7.php" class="tooltipped" data-position="left" data-delay="50" data-tooltip="Add Section">
				...
				<table class="striped">
					...
					<tbody>
						<?php 
						$grd7_query = mysqli_query($con, "SELECT * FROM grd7 ORDER BY section ASC");

						if (mysqli_num_rows($grd7_query ) > 0) {
							

							while($row = mysqli_fetch_array($grd7_query )) {
						?>
						<tr>
								...
								<td></td>
								<td>
							<a href="grade7.php?delete=1&grd7_id=<?php echo $row["grd7_id"];?>" onclick="return confirm('Do you want to delete this?')" 	class="tooltipped" data-position="top" data-delay="50" data-tooltip="Delete">
								<i class="fa fa-trash fa-4x icon-pad-5 " style="color:#f44336;"></i>
							</a>
							<a href="grade7.php?edit=1&grd7_id=<?php echo $row["grd7_id"];?>" class="tooltipped" data-position="top" data-delay="50" data-tooltip="Edit">
								<i class="fa fa-pencil-alt fa-4x icon-pad-5 " style="color:#2196f3; margin-left:10px;"></i>
							</a>
							<a href="grade7.php?add=1&grd7_id=<?php echo $row["grd7_id"];?>" class="tooltipped" data-position="top" data-delay="50" data-tooltip="Add New Adviser">
								<i class="fa fa-plus-circle fa-4x icon-pad-5 " style="color:#4caf50; margin-left:10px;"></i>
							</a>
							<a href="grade7.php?view=1&grd7_id=<?php echo $row["grd7_id"];?>" class="tooltipped" data-position="top" data-delay="50" data-tooltip="view">
